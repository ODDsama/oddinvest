...
// Правити це число можна за спостереженням, не за смаком.
//
// Множник, а не константа, з тієї ж причини, що й скрізь: тести з pause=0
// лишаються миттєвими.
const quotesPauseFactor = 4

// quotesCap — скільки паперів максимум за одне натискання.
//
// Стеля не про сервер, а про людину: прохід синхронний, і сто ISIN — це
// сто з гаком секунд, поки сторінка мовчить. Шістдесят покривають
// портфель разом із видимим верхом рейтингу, а решта підтягнеться
// наступним натисканням, коли зміниться склад порад.
const quotesCap = 60
