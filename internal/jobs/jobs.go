...
// «факт vs модель».
//
// Рядок збирає store.SnapshotOf — те саме відображення, яким жива точка
// кривої «Як росте» будує сьогоднішній день (engine.SnapshotSeries). Доти
// воно жило тут літералом, і друга копія розійшлася б на першій новій
// колонці.
func (r *Runner) Snapshot(ctx context.Context) error {
	...
	today := domain.NewDate(time.Now().In(r.loc))
	return r.st.SaveSnapshot(ctx, store.SnapshotOf(today, doc))
}
