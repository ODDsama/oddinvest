...
// Купон ОВДП — рядок з ISIN і сумою в Дебеті, як у виписці 23.09.2026.
// Податку в нього немає, і чужий податок, що стоїть поруч, до нього не
// липне: він лишається самотнім і йде в пропуски.
func TestInzhurCoupon(t *testing.T) {
	res, err := ParseInzhur([][]string{
		{"Дата", "Тип операції", "Вид цінного паперу", "Дебет", "Кредит"},
		{"46288.44583333333", "Нарахування купону", "ОВДП UA4000238976", "78.4", ""},
		{"46288.44583333334", "Сплата податку", "ОВДП UA4000238976", "", "1.00"},
		{"46288.4", "Нарахування купону", "ОВДП без номера", "10", ""},
	})
	if err != nil {
		t.Fatal(err)
	}
	c := find(t, res, "coupon")
	if c.Date != "2026-09-23" || c.Fund != "UA4000238976" || c.Amount != 7840 || c.Tax != 0 {
		t.Errorf("купон: %+v", c)
	}
	if len(res.Rows) != 1 {
		t.Errorf("мав бути один рядок купона: %+v", res.Rows)
	}
	// купон без ISIN і самотній податок
	if len(res.Skipped) != 2 {
		t.Errorf("пропуски: %+v", res.Skipped)
	}
}
