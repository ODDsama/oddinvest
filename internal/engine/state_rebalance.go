...
// що в розкладці надходження (allocate.go) і на маршруті:
// подушка → цілі → види. Саму базу складає той, хто викликає.
//
// ПОВЕРТАЄ need — недобір кожного рядка до його частки, у гривнях, з
// індексами рядків. Не «про запас»: другий прохід розкладки (той, що
// пропонує залишок тим, хто ще недобирає) упорядковує приймачі саме за цим
// числом, і порахувати його на місці означало б завести ДРУГЕ означення
// того, з чого вже вирахувані бюджети. Розійшлись би вони рівно тоді, коли
// потреб більше, ніж грошей, — тобто в найцікавішому випадку.
//
// nil, коли ділити нема чого: порожній зріз і «усі потреби нульові» — різні
// стани, і другий прохід мусить їх розрізняти.
func spreadMonth(rows []state.RebalanceRow, avail, kindMajor float64) []float64 {
...
func buildRebalance(in rebalanceInput) rebalancePhase {
	...
	for _, cur := range []string{money.USD, money.EUR} {
		...
		out.Rebalance = append(out.Rebalance, state.RebalanceRow{
			...
			FillPct:         domain.Round2(fillPct(curUAH, targetUAH)),
	...
	for _, k := range kindTargets {
		...
		row := state.RebalanceRow{
			...
			TargetPct: domain.Round2(*k.target), CurrentPct: domain.Round2(currentPct),
			...
			FillPct:    domain.Round2(fillPct(k.nowUAH, targetUAH)),
		}
		if k.unit > 0 {
		...
		switch k.key {
		case "bonds":
			row.TransitPct, row.TransitUAH = domain.Round2(transitPct), state.Major(transitCarveUAH, money.UAH)
		case "deposits":
	...
	for _, k := range kindTargets {
		...
		row := state.RebalanceRow{
			Dimension: "kind", Key: k.key, Currency: money.UAH,
			CurrentPct: domain.Round2(k.nowUAH / kindMajor * 100),
			CurrentUAH: state.Major(k.nowUAH, money.UAH),
	...
	addConc := func(dim, key, label string, amount, base, limit float64) {
		...
		row := state.ConcentrationRow{
			...
			AmountUAH: state.Major(amount, money.UAH), SharePct: domain.Round2(share), LimitPct: limit,
		}
		if share > limit {
