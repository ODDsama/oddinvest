...
func (e *Engine) PlanTimeline(ctx context.Context, now time.Time) (timelineDoc, error) {
	...
	actions, err := e.st.ListPlanActions(ctx)
	if err != nil {
		return timelineDoc{}, err
	}
	...
	termDeposits, _ := e.st.ListTermDeposits(ctx) //nolint:errcheck
	funds, _ := e.st.ListFunds(ctx)               //nolint:errcheck
	npfAccounts, _ := e.st.ListNPFAccounts(ctx)   //nolint:errcheck // те саме: НПФ може не бути
	...
	// (api/handlers_positions.go). Горизонт дивідендів тут ширший за річний,
	...
	snaps, _ := e.st.ListSnapshots(ctx, today.AddMonths(-planHistoryMonths-1), today) //nolint:errcheck
