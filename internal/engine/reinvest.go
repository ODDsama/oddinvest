...
//
// САМ ПРАПОРЕЦЬ ЛИШАЄТЬСЯ ФАКТОМ ПРО АУКЦІОНИ, а ось ПОНИЖУВАТИ за ним
...
// даних UA4000235865 коштував 1 077,15 і за дев'ять днів повертав 1 081,75
// — 0,43 %, розтягнуті на рік, тобто 18,9 %. Одна гривня ціни рухала це
// число на 4,4 в.п.; на 549-денному папері та сама гривня рухає його на
// 0,23 в.п. Ставити такий рядок у список, упорядкований дохідністю,
// означає порівнювати вимір із шумом, роздутим у двадцять разів.
//
// ЧИСЛО НЕ З ГОЛОВИ: у застосунку вже є відповідь на це саме питання —
// xirrMinMoneyDays = 30 (state_builder.go, «Ануалізація на коротких
// горизонтах дає сміттєві сотні відсотків»). Другий поріг на ту саму
// задачу розійшовся б із першим на першій же правці.
//
// Друга, незалежна причина: брокери знімають папери з продажу перед
// погашенням. Власник побачив у пораді папір, якого вже немає ні в
// Приват24, ні в Inzhur, і був правий — купити його справді не можна.
//
...
func suggMatKey(s suggestion) string {
	if s.Maturity == "" {
		return "9999-12-31"
	}
	return s.Maturity
...
// suggUnpriced — чи стоїть головне число цього рядка на наближенні.
//
// ДОТИ ЦЕ БУЛО ПРОСТО `stale`, і воно було правильним рівно доти, доки
// ринкової ціни не існувало взагалі. Довід був такий: ціна рахується за
// номіналом, тобто за первинним розміщенням; для паперу, який розміщують
// сьогодні, це чесне наближення, а для того, якого рік не розміщували, —
// вигадка, і рядок вартий менше.
//
// Відколи ціну приносить джерело (0059), довід розпався надвоє. Факт «на
// аукціонах не розміщувався» лишився правдою й далі їде в причину рядка —
// це справжня обставина, купити такий папір можна лише на вторинному
// ринку. Але ПОНИЖУВАТИ за ним більше не можна, коли ціна ринкова: ми
// знаємо, скільки цей папір коштує сьогодні, і аукціон річної давнини до
// цього знання стосунку не має. Інакше застосунок карав би рядок за
// невпевненість, якої в нього вже немає.
//
// Для паперу без свіжої ціни все лишається як було: там головне число
// справді виведене з номіналу, і `stale` каже про нього правду.
func suggUnpriced(s suggestion) bool {
	return s.stale && s.CostBasis != CostBasisMarket
}

func suggPlanScore(s suggestion) float64 {
...
// orderKey — ЯКОЮ ЛІНІЙКОЮ міряти вигоду при впорядкуванні.
//
// Ортогональна до rank: той каже, ЗА ЧИМ сортувати (план, ставка, строк,
// драбина), ця — яким числом міряти саму вигоду. Типово реальна: тільки
// вона розсуджує гривневий папір під 16% і доларовий під 4.5%.
//
// Номінальна тут не «неправильна», а інша: вона відповідає на питання
...
func lessSuggestion(a, b suggestion, rank, order string, byWallet bool) bool {
	...
	// Папір, чиєї ціни ми не знаємо, — нижче за той, чию знаємо, і теж в
	// УСІХ режимах. Підстава інша, ніж у ліміту, і сказати її треба точно:
	// це не судження про папір, а визнання, що головне число рядка
	// тримається на наближенні. Сортувати за впевненістю у ВЛАСНИХ числах
	// застосунок має право; це не порада.
	if as, bs := suggUnpriced(a), suggUnpriced(b); as != bs {
		return bs
	}
	switch rank {
	case "rate":
		...
		if math.Abs(orderKey(a, order)-orderKey(b, order)) > 0.01 {
			return orderKey(a, order) > orderKey(b, order)
		}
	case "short":
		if suggMatKey(a) != suggMatKey(b) {
	...
	default: // plan
		if sa, sb := suggPlanScore(a), suggPlanScore(b); math.Abs(sa-sb) > 0.01 {
			return sa > sb
		}
		if math.Abs(orderKey(a, order)-orderKey(b, order)) > 0.01 {
			return orderKey(a, order) > orderKey(b, order)
...
func (e *Engine) ReinvestSuggestions(ctx context.Context, now time.Time,
	...
	for _, b := range bonds {
		...
		// ДОСТУПНІСТЬ: жоден із твоїх брокерів зараз не продає цей папір —
		// поради немає. Ціна від СВОГО брокера і є доказом того, що крок
		// узагалі можна зробити; без неї порада веде туди, куди не зайдеш.
		//
		// Умова вмикається лише свіжим знаком повного обходу
		// (quoteBook.hideUnpriced): доки ми не питали про всі папери,
		// відсутність ціни означає «не питали», а не «не продають».
		if quotes.hideUnpriced() && pick.Best == nil {
			continue
		}
	...
	//
	// Резервні й цільові вклади сюди НЕ входять: вони не стоять у черзі до
	// паперу, а лежать під аварію або під авто. Порахувати їх транзитом
	// означало б вирішити, що черга вже відстояна, і замовкнути про
	// справжню — саме тим коштом, що подушка й папір це різні гроші.
	depByCur := map[string]float64{}
	for _, d := range deps {
		if d.Active(today) && !d.Earmarked() {
	...
	for _, d := range deps {
		...
		//
		// І тільки НЕ ОБІЦЯНІ (Earmarked). Аргумент написаний у шапці
		// api/handlers_reserve.go: злиття подушки з купівельною спроможністю
		...
		// Цільові вклади (0062) виходять звідси з того самого доводу й
		// поповнюються своєю стелею goals_fill_share_pct.
		if !d.Replenishable || d.Earmarked() || !d.Active(today) || d.Principal <= 0 {
			continue
		}
		c := d.Currency
	...
	// БОРГ — і це відхилення від записаного рішення про подушку й цілі.
	//
	// Їх у цей перелік свідомо не додавали, і довід був точний: у них немає
	// ГОЛОВНОГО ЧИСЛА — реальної дохідності, — тож рядок довелося б або
	// лишити без неї, або вигадати. У боргу вона є, і вона ж є його суттю:
	// погашення дає гарантовану ставку, без податку й без ризику ціни. Це
	// єдиний рядок переліку, у якого «дохідність» точно відома наперед.
	//
	// Тому рядок ОДИН на всі борги, а не по одному на борг: у якому саме
	// борзі опиниться гривня, вирішує черга погашення (/api/payoff), і
