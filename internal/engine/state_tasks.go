...
const (
	...
	// банку, і саме вони роблять пороги правдою. Окремої дії «сплати борг»
	// немає: жодна задача її не ставила, і токен жив лише в схемі.
	actPayCard = "pay-card"
)
...
// fxStaleDays — з якого віку курс НБУ вважається застарілим. Чотири дні, а
// не два: курс пʼятниці законно діє до понеділка, а перед святами НБУ
...
func buildTasks(doc *state.Doc, sug []suggestion, src *sources, today domain.Date) []state.Task {
	...
	// ---------- пропущені записи довідника ----------
	// Кривий запис НБУ більше не валить довідник цілком (nbu.Securities),
	// але папір, що випав, мусить бути видно: інакше в пораді його просто
	// немає, і ніхто не знає чому.
	if src != nil && src.nbuSkipped != "" {
		n := strings.Count(src.nbuSkipped, "; ") + 1
		add(state.Task{
			ID: "nbu-skip", Sev: sevWatch, Rank: 21,
			Title: fmt.Sprintf("Довідник НБУ: %d %s не розібрано", n,
				Plural(n, "запис", "записи", "записів")),
			Why: "Ці папери з останнього оновлення довідника пропущено — їхніх ставок і " +
				"графіків виплат немає. Решта довідника оновилась. " + src.nbuSkipped,
		})
	}

	// ---------- курс НБУ ----------
	// Окремо від довідника: курси тягнуться своїм запитом і падають
	// незалежно, а мітка nbu_refreshed_at про них нічого не знає. Вік — за
	// датою самого курсу, а не за часом запуску: у вихідні НБУ віддає
	// пʼятничний, і поріг fxStaleDays це вміщує.
	if src != nil && src.ratesAsOf != "" {
		if d := daysBetween(src.ratesAsOf, today); d >= fxStaleDays {
			add(state.Task{
...
// reinvest.go), тож попередити треба ДО того, як він зникне.
