...
// ProvidesUAH кожного рядка «Що заходить» (api/handlers_plan.go). «Скільки план
...
// buildPlanFree — ті самі рукави, але БЕЗ плану надходжень: відповідь на
// питання «скільки треба вносити, якби плану не було зовсім».
//
// Потрібне, щоб «треба», «дає план» і «заходить фактично» стояли на одній
// основі й порівнювались напряму. Без цього єдине число, яке застосунок
// умів назвати «треба вносити», насправді означало нестачу ПОНАД план — і
// з тією ж назвою стояло поруч із самим планом.
//
// Приймач у sleeveFactory — значення, і build теж бере значення, тож копія
...
//
// А npfContrib, навпаки, ЗАНУЛЮЄТЬСЯ — і це не четвертий виняток, а
// протилежний випадок: вона не самостійна сутність, а друга половина
// того самого plan. Лишити її при зануленому плані означало б зарахувати
// пенсійному внески, яких ліквідний бік не платив.
//
// Наслідок, який треба знати: валюта, що існує ВИКЛЮЧНО завдяки плану
// (нативний потік або set_shares у неї, при нульовій базовій частці й
// порожньому рахунку), у плановільному розрахунку зникає — і це
// правильно, бо питання звучить «а якби плану не було». Валюта, у якій
// лежать гроші, не зникає ніколи: сторож у build дивиться ще й на
// готівку, номінал і фонди.
//
// І чесна межа точності: без плану замок може загнати cash у мінус
// (projection.go це допускає навмисно). Сума зберігається, але відʼємна
// готівка не заробляє, тож для портфеля з великими майбутніми замками
// «треба» виходить трохи ЗАНИЖЕНИМ. Прибирати замки було б гірше — це
// змінило б форму моделі, а не лише її фінансування.
func (f sleeveFactory) buildPlanFree(contribTotal, ratePP float64) []domain.Sleeve {
	f.plan = nil
	f.planNative = nil
	// npfContrib — ДРУГА половина руху внеску в пенсійний, і занулюється
	// вона РАЗОМ із першою. Доти тут стояли лише plan/planNative: дебет
	// зникав, кредит лишався, і базова лінія «якби плану не було»
	// зараховувала пенсійному рахунку гроші, які нізвідки не пішли.
	// Капітал у ній виходив завищеним, а «скільки треба було б вносити»
	// — рівно на стільки ж заниженим.
	//
	// Це виправлення, а не вибір: половини цього руху нероздільні за
	// побудовою (див. коментар над npfContrib у newSleeveFactory), і
	// тримати одну без другої означає або друкувати гроші, або їх палити.
	f.npfContrib = nil
...
func buildProjection(in projectionInput) projectionPhase {
	...
	// Цілі накопичення — той самий виняток, що й резерв, і навіть різкіший:
