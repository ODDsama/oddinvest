...
//
// # ЩО ДАЄ ДОСТРОКОВЕ ПОГАШЕННЯ РОЗСТРОЧКИ — І КОЛИ НЕ ДАЄ НІЧОГО
//
...
//
// Але скасовуються вони НЕ В КОЖНОМУ ДОГОВОРІ, і це не дрібниця умов, а
// межа всієї сторінки. Там, де банк бере комісії за весь строк одразу,
// дострокове погашення не економить нічого: сума та сама, лише віддана
// раніше. Доти прохід уперед вважав скасування правилом і обіцяв власнику
// «на 1 083 ₴ дешевше» на розстрочках, де економія дорівнює нулю, — тобто
// радив дію, СТРОГО ГІРШУ за бездіяльність, і називав її економією.
//
// Тепер відповідь на це питання приходить із договору (Debt.FeeOnPrepay,
// міграція 0049), а правило одне на всі види боргу:
// domain.DebtPrepayCancels. Гроші понад обовʼязкове йдуть ЛИШЕ туди, де
// вони щось скасовують. Наслідок, який виходить сам і теж правильний:
// безвідсоткова розстрочка «частинами» дострокових грошей більше не
// отримує — скасовувати в ній нічого, а віддати раніше означає просто
// віддати раніше.
//
...
func runPayoff(debts []payoffDebt, strategy string, extra int64) payoffRun {
	...
	for month := 0; month < payoffMaxMonths; month++ {
		...
		//
		// Борги, у яких дострокове погашення нічого не скасовує, пропускає
		// не сортування, а ця умова: у списку вони лишаються (їхні
...
//
// НЕПРИДАТНІ ДО ДОСТРОКОВОГО ЙДУТЬ У ХВІСТ, а не викидаються: цей самий
// порядок малює список боргів на екрані (handlePayoff), і борг, зниклий
