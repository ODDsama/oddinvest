...
func (s *Server) importStatement(w http.ResponseWriter, r *http.Request, prof *store.ImportProfile) {
	...
	// ЧОМУ ЗАПИС НЕ В ОДНІЙ ТРАНЗАКЦІЇ. Рядки пишуться по одному, і
	// помилка бази посеред файла лишає імпорт половинним. Це свідомо:
	...
	// колись трапиться насправді — тоді й робити транзакцію, не раніше.
	//
