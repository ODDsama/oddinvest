...
func TestSaleEditAndDelete(t *testing.T) {
	...
	//
	// Саме 404, а не 400. Доти всі хендлери, крім планових, звалювали
	// «немає такого id» у 400, тобто відповідали «ти надіслав дурню» на
	// цілком коректний запит про запис, який просто вже видалено. Різницю
	// видно там, де її читає людина: модалка правки показує повідомлення
	// бекенда дослівно.
	if resp, _ := do(t, "DELETE", srv.URL+"/api/sales/1", ""); resp.StatusCode != http.StatusNotFound {
		t.Errorf("повторне скасування мало повернути 404, маємо %d", resp.StatusCode)
	}
}
...
//
// ЗНІМОК ТУТ ОБОВʼЯЗКОВИЙ, і це не набір заради набору. Відколи бенчмарк
// став одним рядком «Ціни моїх рішень» (engine/rivals.go), він рахується на
// ВІКНІ ЗНАННЯ — від першого добового знімка. Причина знайдена на живих
// даних: у журналі гаманця лежали рухи 2024–2025 років, коли портфеля не
// існувало взагалі (перший лот — липень 2026-го), і половину тих грошей
// давно зняли. Стара повна історія кредитувала долар двома роками
// девальвації на гроші, яких уже немає, — і виходило, що портфель
// програє. Знімок і є та мітка, від якої застосунок узагалі знає, скільки
// в нього було.
...
func TestBenchmarkWithdrawalsReduceIt(t *testing.T) {
	srv, st := testServer(t)
	ctx := context.Background()
	openWindow(t, st, "2025-01-15", store.Snapshot{})
...
// звіряємо з реєстром settings.Keys.
...
// резерву (engine/state_rebalance.go), місце під подушку тримати не треба: у неї
...
func TestBlendedYieldCoversDepositsAndSkipsReserve(t *testing.T) {
	...
	// Єдиний вид у портфелі — вклади, тож зведена дорівнює ставці цього
	// вкладу після податку. Договірна проста дає 16% × (1 − 0.195) =
	// 12.88%, але виплата ЩОМІСЯЧНА, тобто гроші приходять раніше, і в
	// одному зваженому середньому з YTM облігацій вклад мусить стояти
	// ефективною: (1 + 0.1288/12)^12 − 1 = 13.67%.
	//
	// Доти тут стояло 12.88, і саме це число було вадою, а не тест: проста
	// ставка вкладу порівнювалась із IRR облігації в одній колонці.
	if math.Abs(sum.BlendedPct-13.67) > 0.01 {
		t.Errorf("зведена мала дорівнювати ЕФЕКТИВНІЙ ставці вкладу 13.67, маємо %v", sum.BlendedPct)
	}
