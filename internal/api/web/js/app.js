...
//
// Одне уточнення, відколи «Стратегія й мета капіталу» ставить виміряні числа поруч
...
export class OddInvestApp extends HTMLElement {
  ...
  // Перемикання портфеля — ПЕРЕЗАВАНТАЖЕННЯ сторінки, а не перемальовування.
  //
  // Доти оболонка міняла store й малювала заново, а вісім модулів тримали
  // свій стан на рівні модуля: поради помічника (now-view), операції фондів,
  // рахунки НПФ, вибір паперу в маршруті, «чим добрати», пороги перекладання,
  // кеш знімків «Як росте», питання «а якщо докласти» в боргах. Жоден не
  // знав про зміну портфеля, і вибране в одному протікало в інший — рівно
  // до наступного F5. Реєстр скидань довелось би доповнювати кожним новим
  // модулем, і забутий модуль знову мовчки брехав би; перезавантаження
  // скидає все за побудовою. Портфель міняють рідко, а маршрут у хеші й
  // вибір у localStorage переживають його самі.
  _setPortfolio(slug) {
    setPortfolio(slug);
