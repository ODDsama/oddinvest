...
function destHTML(leg) {
  ...
  // Цілі — ДРУГИМИ, за подушкою й перед паперами: так само, як у самому
  // обчисленні. Вирізки боргу між ними більше немає — маршрут не веде
  // гроші в дострокове погашення (довід — у engine/allocate.go).
  ...
  // Поза цільовими частками — не призначення, а гроші, які людина лишила
  // собі (частки видів у сумі менші за сотню). Рядком, бо доти вони просто
  // зникали: «Разом» показувало більше, ніж колонки поруч у сумі.
  if (leg.free_uah > 0) {
    rows.push(`<div class="fine muted">поза частками — <b>${fmtUAH(leg.free_uah)}</b></div>`);
  }
  ...
  // лише грошей. «Поза частками» до них не належить.
  const took = rows.length - (leg.free_uah > 0 ? 1 : 0);
 ...
 *  і платяться з доходу, який до портфеля не доходить (довід — у шапці
 *  route.go, «Борг»). Тут
 *  видно інше: скільки щомісяця йде в борг повз портфель, коли борг під
 *  ставкою закінчується і де платежів
...
function monthsHTML(doc) {
  ...
  const any = rows.some((r) => r.debt_due_uah > 0 || r.card_inst_uah > 0
    || r.debt_left_uah > 0 || r.planned_uah > 0);
  if (!any) return "";
  ...
  const grid = opsGrid({
    cols: [
      ...
      // Колонки «Достроково за маршрутом» більше немає: маршрут веде лише
      // обовʼязкове, а борг у таблиці тане за ГРАФІКОМ.
      {
...
export async function renderRoute(ctx, main) {
  ...
  const body = (doc.legs || []).length
    ? legsHTML(doc) + `
      <div class="sub-xs">Рядок без позначки — <b>зобовʼязання</b>: портфель винен це
        ...
        доході місяця мінус витрати, пропорційно. У поточному
        ...
        Дата «коли вистачить» у «Що взяти» ні оцінок, ні плану не бачить навмисно —
        там одне число, і припущення в ньому стало б невидимим.</div>
      <div class="sub-xs">Порядок і ціна кроку тут <b>сьогоднішні</b>: майбутнього накопиченого купона,
  ...
  const card = `<div class="card">
    ...
    <div class="note">Кожне надходження — те, що портфель уже винен сам собі, і те,
      ...
      «Що взяти». Дрібні надходження чекають одне одного, доки не набереться на цілий
