...
//
// ВИРІЗКИ ПОДУШКИ Й ЦІЛЕЙ ТУТ БЕЗ ПРАПОРЦІВ, на відміну від
...
// ріже бекенд: подушка → цілі. Третій порядок на третьому екрані читався б
// як третє правило. Боргу між ними більше немає — дострокове погашення не
// забирає портфельних грошей ніде (фаза 45).
//
...
function cutsHTML(t) {
  ...
  return `${out.join("")}<div class="sub-xs">Ці гроші забирають своє ПЕРЕД видами —
    ...
    звідси: подушка й цілі записуються рухом на своїй сторінці.</div>`;
}
...
export function topupHTML(ctx, res) {
  ...
  return `<div class="card">
    ...
    <div class="note">Скільки з решти місяця куди, щоб частки вирівнялись. Порядок
      той самий, що в самому обчисленні: подушка → цілі → види, і бюджет
      виду — це «скільки йому бракує до його частки» (та сама колонка «на
      вирівнювання» в <a class="lnk" href="${routeFor("work/buy/main")}">«1 · Скільки й куди»</a>).
      Частки міряються від портфеля, ЯКИМ ВІН СТАНЕ з рядками плану, — тож те, що
      вже заплановано, вдруге не пропонується. Ціна кроку тут «номінал + накопичений купон», у
