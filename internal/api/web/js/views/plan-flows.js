...
export function usesFields(uses) {
  ...
  return `<div class="row-h" title="Куди цим грошам МОЖНА. Скільки саме туди піде,
    ...
    <div class="sub-xs muted">Галочки ділять ЛИШЕ ті гроші, що доходять до
      портфеля — тобто частку вище. При частці 0% вони не впливають ні на що:
      ділити нема чого, а решта доходу лишається там, куди прийшла.</div>
    <div class="sub-xs muted">«Борг» тут означає «з цього надходження можна різати
      на ДОСТРОКОВЕ погашення» — наприклад, гасити розстрочку із зарплати, але не з
      купонів. Скільки саме різати, каже стеля в «Політиці → Борг»; сама галочка не
      робить нічого. Обовʼязкові платежі до неї не стосуються — вони не вибір, і
      застосунок віднімає їх від грошей місяця сам.</div>`;
}
...
export function revisionsHTML(revs) {
  ...
  // кожному рядку рівно для цього (engine/plan_timeline.go,
