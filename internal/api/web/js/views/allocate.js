...
// від якої міряються цільові частки видів (довід — у engine/allocate.go).
...
// з того, що після неї лишилось (engine/allocate.go), і той самий порядок
...
export async function openAllocate(ctx, opts) {
  ...
  const fields = routedHTML(routed)
    ...
    + `<div class="sub-xs">Поділ між видами — той самий, що в картці «Куди йдуть гроші
       ...
       твій, із налаштування «Порядок у ‹Що взяти›». Ціна кроку тут «номінал + накопичений купон»,
