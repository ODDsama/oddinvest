...
export function yieldMixCard(ctx) {
  ...
  // Відʼємна готівка теж рядок: без неї сума стовпця перестає дорівнювати
  // капіталу, хоч картка це прямо обіцяє нижче.
  if (Math.abs(s.account_uah || 0) > 0.005) {
  ...
  const body = rows.map((r) => {
    ...
    if (!r.target_pct) {
      ...
      const why = r.key === "deposits"
        ...
        : r.key === "cash"
          // Рядок, якого тут бракувало найдовше. Примітка під карткою вже
          // казала, що невкладена готівка лишається в знаменнику, — але
          // самої готівки в переліку не було, і недобір усіх видів разом
          // читався як помилка налаштувань, а не як «гроші ще лежать».
          ? "цілі немає й не буде: це не вид, а гроші, які ще не стали видом — "
            + "рівно на їхню частку всі види разом і стоять нижче своїх цілей"
          : "показано довідково";
...
// рівнями. Число не змінилось джерелом: віха «Обіграв просто долари»
// бере його з того самого рушія, тож двох рахунків одного числа немає.
...
// теоретичне.
//
// ГОЛОВНЕ ЧИСЛО ТУТ — «ПІД РУКОЮ», А НЕ РАХУНОК БРОКЕРА. Доти картка
// казала «доступно 9,87 ₴» портфелю, поруч із яким лежала подушка
// готівкою й відкладене під цілі, — обидві суми бекенд надсилав від
// самого початку, і не читав їх жоден екран.
//
// Це саме про доступність, а не про дозвіл: картка не каже, що подушку
// МОЖНА витратити, — вона каже, що діставати її нізвідки не треба.
// Рахунок лишається окремим підрядком, бо на now_uah == account_uah
// тримається звірка зі звітом про рух коштів.
export function liquidityCard(ctx) {
  const l = (ctx.summary || {}).liquidity;
  if (!l) return "";
  const now = l.now_uah || 0;
  const reserve = l.reserve_uah || 0, goals = l.goals_uah || 0;
  // Старший бекенд поля не надсилає — тоді головним лишається рахунок і
  // картка виглядає рівно так, як виглядала.
  const avail = l.available_now_uah == null ? now : l.available_now_uah;
  const sub = (t) => `<div class="sub">${t}</div>`;
  const hint = fmtUAH(avail);
  return `<div class="card">${disclosure("liquidity", "Ліквідність", `
    <div class="tiles flush">
      ${reserve > 0 || goals > 0
      // Розкладка під числом обов'язкова: сума, яку нема з чого зібрати,
      // читається як помилка застосунку — надто коли рахунок порожній, а
      // «під рукою» тисячі.
      ? tile("Під рукою", fmtUAH(avail),
        `${sub(`з них на рахунках ${fmtUAH(now)}`)}
         ${reserve > 0 ? sub(`подушка готівкою ${fmtUAH(reserve)}`) : ""}
         ${goals > 0 ? sub(`під цілі ${fmtUAH(goals)}`) : ""}`, { hero: true })
      : tile("Зараз", fmtUAH(now), sub("на рахунках"))}
      ${tile("За 30 днів", fmtUAH(l.in_30_uah), sub("разом із виплатами"))}
      ${tile("За 90 днів", fmtUAH(l.in_90_uah), sub("разом із виплатами"))}
      ${l.locked_uah > 0 ? tile("Замкнено", fmtUAH(l.locked_uah),
        ...
        //
        // Подушка — таким самим підрядком і з того ж міркування: рунга
        // лишається в спільному числі (для «коли звільниться» вклад є
        // вклад), але без підпису картка мовчала б про те, що частина
        // замкненого — власні аварійні гроші, які просто ще не дозріли.
        `${l.locked_npf_uah > 0
          ? sub(`з них у НПФ ${fmtUAH(l.locked_npf_uah)} — до пенсійного віку`) : ""}
         ${l.locked_reserve_uah > 0
          ? sub(`з них подушка ${fmtUAH(l.locked_reserve_uah)} — резервні вклади`) : ""}
         ${l.unlock_date ? sub(`найближче відкриється ${esc(l.unlock_date)}`) : ""}`) : ""}
      ${l.breakable_uah > 0 ? tile("Можна забрати достроково", fmtUAH(l.breakable_uah),
        ...
        // дістати можна.
        `${sub("відкличні вклади: тіло повернуть, відсотки згорять")}
         ${l.breakable_reserve_uah > 0
          ? sub(`з них подушка ${fmtUAH(l.breakable_reserve_uah)}`) : ""}`) : ""}
    </div>
    <div class="muted fine">«Під рукою» — гроші на рахунках, готівка подушки й відкладене
      під цілі разом: усе, що не треба нізвідки діставати. Це про ДОСТУПНІСТЬ, а не про
      дозвіл витрачати — подушка тут не стає купівельною спроможністю й у поради «що
      купити» не входить. Рахунки названо окремим підрядком навмисно: на їхній рівності
      з балансом брокерів тримається звірка зі звітом про рух коштів.<br><br>
      Вікна накопичувальні: «за 90 днів» уже містить «за 30». До «під рукою» вони додають
      купони, погашення й тіла вкладів, що гасяться у вікні.
      ...
      ставлять на самому вкладі за договором. Резервні вклади проходять ці дві плитки
      звичайними строковими й позначені підрядком — коли саме дозріє наступна рунга,
      каже сходинка доступу в картці подушки.<br><br>
...
export function rateRiskCard(ctx) {
  ...
  const priceBlock = rr.duration_years ? `
    ...
    <div class="muted mt-sm fine">Модифікована дюрація показує, на скільки %
      ...
      їх нікуди, сума погашення записана в договорі.
      <br>Оцінка ЛІНІЙНА: справжня крива ціни трохи вигнута, тож на великих зсувах падіння
      насправді менше за показане, а зростання більше. На ±1 п.п. різниця мала, на ±2 —
      кілька десятих відсотка.</div>` : "";
