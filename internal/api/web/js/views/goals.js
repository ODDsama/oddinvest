...
//
// ЧИМ ЦЕ НЕ РЕЗЕРВ І НЕ МЕТА КАПІТАЛУ. Три схожі слова живуть у застосунку
// поруч, і сплутати їх легко:
//
//   резерв        — гроші на чорний день, без суми й без дати; питання
//                   «чи вистачить прожити»;
//   мета капіталу — куди виводить портфель разом («План → Мета капіталу
//                   й прогноз»); питання «коли я стану незалежним»;
...
export function goalTilesHTML(g) {
  ...
  return `<div class="card">
    ...
    <div class="note">Гроші, відкладені на названу річ у названу дату. У купівельну
      спроможність не входять, як і резерв, і помічник реінвесту про них не знає — але
      від резерву відрізняються тим, що з ними буде: подушку тримають, щоб НЕ витратити,
      ціль — щоб витратити. Лежати вони можуть і на вкладі: тоді ціль не програє
      інфляції просто так.</div>
...
function paceHTML(g, sym) {
  ...
  // Другий темп зʼявляється рівно тоді, коли бекенд його порахував:
  // гривнева ціль, є дедлайн і є ряд ІСЦ. Вирок «встигаю» стоїть САМЕ на
  // ньому, тож без цього рядка на екрані не було б числа, яким цей вирок
  // виноситься (валютна ціль лишається на першому — індексу цін країни
  // валюти в застосунку немає).
  const future = g.required_future_native > 0;
  return `<div class="kv"><span class="muted">До ${esc(g.due_date)}</span>
    ...
    <div class="kv"><span class="muted">Треба відкладати${future
    ? " <span class=\"muted\">(за сьогоднішньою ціною)</span>" : ""}</span>
      <b>${fmtCur(g.required_native || 0, sym)}/міс</b></div>
    ${future ? `<div class="kv"><span class="muted">Треба відкладати за ціною
      ${esc((g.due_date || "").slice(0, 4))} року</span>
      <b>${fmtCur(g.required_future_native, sym)}/міс</b></div>` : ""}
    ...
    : `<span class="t-ok">темп тримається ✅</span>`}${future
    ? " — рахується за ціною року дедлайну, тобто за другим рядком" : ""}</div>
 ...
 *  найдовших цілях.
 *
 *  ЦЕ БІЛЬШЕ НЕ ДОВІДКА. Доти тут стояло, що числа вище майбутньої ціни
 *  «не враховують навмисно», і на цьому все й спинялось: вирок «встигаю»
 *  порівнював сьогоднішній темп із сьогоднішньою ціною, хоч дедлайн у
 *  майбутньому. Тепер вирок і стеля наповнення стоять саме на майбутній
 *  ціні, а сьогоднішні числа лишились ПОРУЧ — не заради сумісності, а
 *  тому, що ціль тримають у голові в сьогоднішніх грошах, і різниця між
 *  двома числами і є те, що варто побачити.
 *
...
function futureHTML(g, sym) {
  ...
  <div>
    ...
    <div class="kv"><span class="muted">Зібране працює під</span>
      <b class="${g.rate_pct > 0 ? "t-ok" : "t-warn"}">${g.rate_pct > 0
    ? pct(g.rate_pct) + " чистими" : "0% — лежить готівкою"}</b></div>
    <div class="sub">Ціль задана в сьогоднішніх грошах, а купувати за неї будеш
      ...
      Саме на цій ціні стоять «відстаю» й стеля наповнення: дедлайн у майбутньому, тож
      міряти його сьогоднішньою ціною означало б порівнювати різні роки.
      ${g.rate_pct > 0
    ? `Зібране при цьому росте на власну ставку — через це потрібний темп нижчий,
        ніж був би в шухляді.`
    : `Зібране при цьому не працює зовсім: поки воно лежить готівкою, ціль дорожчає, а
        гроші — ні. Вклад під ціль (у формі вкладу) це змінює.`}</div>
  </div>`;
}
