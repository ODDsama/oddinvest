...
function planBuyFields(ctx, row = null) {
  ...
  return [
    ...
    `<div class="note">Порожня дата або дата в межах цього місяця — «купую
      зараз»: рядок одразу видно в частках, драбині й лімітах. Дата з
      наступного місяця переносить його в прогноз — сьогоднішні числа він
      не рухає, зате рухає ціль і точку незалежності.</div>`,
  ];
}
