...
function planBuyFields(ctx, row = null) {
  ...
  return [
    ...
    // порожньо означає «купую зараз», а дата з наступного місяця переводить
    // рядок у прогноз. Підставлений сьогоднішній день зробив би «зараз»
    ...
    // ТУТ СТОЯЛА ПІДКАЗКА ПРО ДАТУ, і вона переїхала за кнопку «i» на
    // заголовку картки (now-view.js). Текст не скорочено — його там уже
    // не було потреби писати: абзац «Дата вирішує все» в INFO.basket
    // багатший на кожному пункті, бо додає дюрацію, довід про календарну
    // сітку прогнозу й випадок простроченої дати, якого інлайн не
    // згадував зовсім.
    //
    // Той самий рух і з того самого доводу вже зроблено з дев'ятьма
    // поясненнями «Налаштувань» (див. шапку розділу в info.js): довга
    // сіра проза між заголовком картки й полем — найдовший скрол у
    // застосунку, і читати його доводиться щоразу, коли треба поправити
    // одне число.
    //
    // planBuyFields живить і МОДАЛКУ правки, у якої «i» немає й не буде:
    // давати openEdit слот під довідку заради одного клієнта — порожній
    // шов (CLAUDE.md §3), а модалка все одно відкривається зі сторінки,
    // де кнопка на екрані.
  ];
}
...
// «Коли» одним словом. Чотири стани, і кожен щось означає для чисел
// нижче.
//
// Прострочене підписуємо вголос: воно рахується як «зараз» (гроші за ним
// досі не витрачені), і мовчазне «зараз» приховало б, що ти щось відклав
// і забув. Рядок ЦЬОГО місяця — теж «зараз», і його теж підписуємо, бо
// інакше дата попереду виглядала б як обіцянка не чіпати сьогоднішніх
// чисел, а вона їх уже чіпає.
//
// Обидва прапорці приходять із сервера й тут не рахуються: «сьогодні» в
// браузері й на сервері — різні дати в різних поясах, і підпис
// розійшовся б із класифікацією, яка дала самі числа.
function whenCell(l) {
  if (!l.buy_date) return `<span class="muted">зараз</span>`;
  if (l.future) return esc(l.buy_date);
  if (l.overdue) return `${esc(l.buy_date)} <span class="t-warn fine-xs">· прострочено</span>`;
  return `${esc(l.buy_date)} <span class="muted fine-xs">· цього місяця</span>`;
}
...
/** Картка плану: таблиця й підсумок.
 *
 *  НЕСТАЧІ ТУТ БІЛЬШЕ НЕМАЄ, і рядка «грошей вистачає» теж. Обидва
 *  міряли сьогоднішній залишок брокера, а план купівель міряється
 *  ПЛАНОВИМИ грошима: якщо на рахунку бракує, він поповниться з
 *  надходжень раніше, ніж покупка станеться. Тобто «у mono бракує 1 000»
 *  було тривогою про стан, який не настане, а «грошей вистачає» —
 *  обіцянкою, якої картка дати не може.
 *
 *  Картка, що мовчки втратила рядок, читається як зламана, тож абзац
 *  лишається тут; повний довід — над basketDoc у engine/state_plan_buys.go.
 *  Перевірка залишку жива й стоїть там, де платіж записують СПРАВДІ:
 *  форми лота, вкладу, поповнення й НПФ. */
