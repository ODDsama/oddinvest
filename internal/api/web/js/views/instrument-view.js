...
function nextForPositionHTML(ctx, kind, row) {
  ...
  if (kind === "fund" && row.next_payout) {
    // Фонд платить того дня в обох випадках — різниця в тому, КУДИ. Мовчати
    // про це не можна: маршрут показує лише решту, і без підпису тут картка
    // й маршрут виглядали б як два різні числа про одну виплату.
    return `<div class="card"><h2>Найближча виплата</h2>
      ${line(row.next_payout, "дивіденд")}
      ${row.kind === "drip"
    ? `<div class="sub">Фонд утримує виплату й докуповує на неї свої ж
        сертифікати цілими штуками. На рахунок падає лише решта — саме її
        веде «Маршрут грошей».</div>` : ""}</div>`;
  }
