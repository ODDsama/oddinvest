 ...
 *
 *  zero: true — нуль ЛЕЖИТЬ НА ШКАЛІ. Так читаються СУМИ: стовпчик росте
 *  з підлоги, і «наскільки він високий» має сенс лише відносно нуля. Для
 *  невідʼємних даних нуль — сама підлога, як було завжди. Відʼємні дані
 *  підлогу опускають: доти вона стояла на нулі намертво, і «Внесено» на
 *  «Факт vs план» після зняття з резерву падало за полотно, просто на
 *  підписи дат, — хоча мінус там законний: гроші справді пішли назовні.
 *
...
function niceScale(values, { zero = false } = {}) {
  ...
  // Нуль на шкалі при відʼємних даних — розтягуємо межі до нього, а далі
  // та сама кратна кроку сітка, що й нижче. Межі кратні кроку, тож нуль
  // сам виходить поділкою, і лінію нуля видно (axisY її виділяє).
...
function axisY(scale, x0, x1, Y, fmt) {
  ...
  scale.ticks.forEach((v, i) => {
    ...
    // рядок від рядка, тобто --oi-rule-hair, найтихіший рівень. Коли шкала
    // сягає нижче нуля, так само виділена й лінія нуля: без неї від'ємну
    // частину кривої не відрізнити від просто низької.
    const edge = i === 0 || (Math.abs(v) < 1e-9 && scale.lo < 0);
    grid += `<line x1="${x0}" y1="${gy.toFixed(1)}" x2="${x1}" y2="${gy.toFixed(1)}"`
      + ` stroke="${edge ? "var(--oi-border)" : GRID}" stroke-width="1"/>`;
...
export function seriesChart(dates, series, { width = 760, height = 300, minWidth = 520, label = "" } = {}) {
  ...
  if (n) {
    // Остання дата йде поза кроком, і регулярна мітка перед нею може
    // стати впритул: «09-22» і «09-23» злипались у «09-2223». Злипання
    // стало майже гарантованим, відколи остання точка — жива (сьогодні
    // поруч із учорашнім знімком), тож ближчу за пів кроку мітку прибираємо.
    const prev = Math.floor((n - 1) / step) * step;
    if (prev > 0 && prev < n - 1 && n - 1 - prev < step / 2) marks.delete(prev);
    marks.add(n - 1);
  }
  // «09-23» без року — доки крива вміщається в один рік. Щойно вона його
  // перетинає (перемикач «усе» на «Капіталі»), той самий підпис значив би
  // два різні дні, тож рік повертається двома цифрами: «26-09-23».
  const cut = n && dates[0].slice(0, 4) !== dates[n - 1].slice(0, 4) ? 2 : 5;
  [...marks].sort((a, b) => a - b).forEach((i) => {
    xlabels += `<text x="${x(i).toFixed(1)}" y="${height - 14}" text-anchor="${xAnchor(x(i), P.l, width - P.r)}" font-size="11" fill="${AXIS}">${esc(dates[i].slice(cut))}</text>`;
  });
