...
func buildMonth(src *sources, hold domain.Holdings, rates fx.Rates,
	...
	// три щомісячні внески покривають три місяці, тоді як від першого до
	// сьогодні минуло лише два. Ділення на проміжок завищувало темп у
	// півтора раза (15 000 за 60 днів давали 7 610 ₴/міс замість 5 000). Та
	// сама поправка знімає й вибух на старті: одне поповнення сьогодні дає
	// знаменник 1, а не 0.1, тож окремий поріг не потрібен.
	//
	// Чого тут свідомо НЕ зроблено: гроші під матрацом у прогнозі «За
	// фактом» реінвестуються під дохідність портфеля, хоч насправді не
	// працюють. Це та сама вада, що вже є в «Плані» (plan_provides_uah теж
	// не питає, куди підуть гроші), і лікувати її треба обом одразу — інакше
...
func buildMonthPlan(src *sources, rates fx.Rates, today domain.Date,
	...
	// ОБОВʼЯЗКОВІ ПЛАТЕЖІ ГАСЯТЬ СПЕРШУ НЕПОРТФЕЛЬНІ ГРОШІ, і лише
	// переповнення доходить до плану.
	//
	// Доти вони віднімались від портфельних нарівні з витратами, і на
	// бойових даних це давало подвійне віднімання. Власник виставив двом
	// найбільшим потокам частку в портфель 0 % саме тому, що ці гроші
	// йдуть на життя й на борг: 222 800 ₴ лишались на картці, 9 000 ₴
	// доходили до портфеля, а розстрочка на 2 407,80 ₴ віднімалась від
	// ДРУГИХ. План місяця казав 6 592,20 ₴ там, де планові надходження —
	// 9 000 ₴, і жоден екран не називав різниці.
	//
	// Попередження при MonthPlan.DebtDueUAH («не вписуй платежі вдруге у
	// витрати й не занижуй ними invest_bp») застосунок порушував сам: у
	// нього не було поняття, з якого кошика платять. Той самий довід уже
	// стоїть нижче, у шапці debtDueForMonth, — але доти його застосували
	// лише до розстрочок, ПРИВʼЯЗАНИХ ДО КАРТКИ. Решта обовʼязкового
	// лишалась на портфельних грошах без жодної причини.
	//
	// Кошик — OnCardUAH: валовий дохід мінус те, що дійшло до портфеля.
	// Коли всі потоки на 100 %, він нульовий і не змінюється НІЧОГО —
	// саме той випадок, у якому це рахувалось правильно й доти.
	//
	// ВИТРАТИ ТАК НЕ ПОВОДЯТЬСЯ, і це рішення, а не забудькуватість. У
	// витратного потоку є ВЛАСНА частка в портфель: поставити їй 0 % і
	// означає сказати «ця витрата не з портфельних грошей»
	// (planFlowAmount множить на неї й витрату теж). У боргу такої ручки
	// немає й бути не може — розстрочка не потік, — тож те, що для
	// витрати вибір користувача, для боргу мусить бути правилом.
	//
	// ЧОГО З OnCardUAH НЕ ВІДНІМАЄТЬСЯ: побут. Це валовий залишок, а не
	// профіцит картки — з нього ж і живуть. Тож у місяці, де картка й так
	// вибрана до ліміту, застосунок тут оптимістичний рівно на суму
	// обовʼязкового. Питання «чи вистачає карткових грошей» має власний
	// екран (buildDebtExit), і зводити два контури в один тут означало б
	// завести третє означення побутових витрат.
	//
	// У РЕЖИМІ after (місяць звірки картки) обовʼязкове датою НЕ
	// фільтрується, а валове й портфельне — фільтруються, тож
	// переповнення в тому виклику завищене. Читачі того виклику
	...
	// ньому бере участь. Тут лишається його довід: це «все інше» — гроші,
	// які нікуди не розподіляються, бо просто лишаються там, куди прийшли:
	// на картці, і витрачаються з неї.
	//
...
//
// «ІНШІ ГРОШІ» ТЕПЕР НАЗВАНІ: це OnCardUAH — дохід, який до портфеля не
// доходить (buildMonthPlan вище). Доти той самий довід застосовувався
// лише до половини випадків: розстрочку картки з портфельних грошей
// прибрали, а самостійну й мінімалку — ні.
//
// АСИМЕТРІЯ, ЯКУ ТРЕБА ЗНАТИ: CardExitInput.InstallmentUAH (domain/debt.go)
// рахує лише розстрочки, ПРИВʼЯЗАНІ ДО КАРТКИ, тож ліміт витрат ще не
// бачить, що на ті самі карткові гроші претендує й самостійна розстрочка.
// Одна гривня обіцяна двом контурам. Не виправлено тут навмисно: правка
...
// savingsRatePct — норма заощаджень: яка частка валового доходу справді
// відкладається.
//
// Чисельник — темп нових грошей за трьома журналами зовнішніх грошей
// (гаманець, подушка, цілі); знаменник — валовий дохід місяця. Обидва вже
// пораховані, і жодне з них тут не перераховується: друге означення
// «скільки зайшло ззовні» коштувало б рівно того, що коштувало минулого
// разу, коли їх було три.
//
// Мовчить без плану доходу — тобто число не існує, доки нема з чим
// ділити. Нуль у цьому місці читався б як «нічого не відкладаю», а це
// зовсім інше твердження.
//
// Стеля 100% НЕ ставиться навмисно: відкладати більше, ніж заробив цього
