...
//
// База обох — гроші ПІСЛЯ резерву: подушка забирає своє першим, і ділити
// суму, з якої вона ще не відрахована, означало б порадити ті самі гроші
// двічі.
func spreadMonth(rows []state.RebalanceRow, mp *state.MonthPlan, totalMajor float64) {
	if mp == nil || totalMajor <= 0 {
		return
	}
	avail := mp.PlanUAH - mp.ReserveUAH
	if avail <= 0 {
		return
	}
	// Капітал після місяця — разом із тим, що піде в резерв: він теж капітал,
	// і цілі за видом міряються від усього.
	after := totalMajor + mp.PlanUAH
...
func buildRebalance(in rebalanceInput) rebalancePhase {
	...
	spreadMonth(out.Rebalance, in.MonthPlan, totalMajor)
