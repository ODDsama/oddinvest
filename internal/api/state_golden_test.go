...
func richPortfolio(t *testing.T, srv string, st *store.Store) {
	...
	// Ряд ІСЦ: сто тридцять два місяці, як його набирає бекфіл. Числа
	// нерівні навмисно — рівний ряд дав би перцентиль 50 на будь-якому
	// вікні, тобто перевірку, яка не може не зійтись.
	//
	...
	for i := 0; i < 132; i++ {
		if err := st.SaveCPI(ctx, store.CPIPoint{
			Period: cpiMonth,
			MoMBP:  int64(40 + (i%9)*25),
			YoYBP:  int64(500 + (i%6)*280),
		}); err != nil {
			t.Fatal(err)
		}
		md, err := domain.ParseDate(cpiMonth + "-01")
		if err != nil {
			t.Fatal(err)
		}
		cpiMonth = string(md.AddMonths(-1))[:7]
	}
	...
	// Третя ціль існує рівно заради ОДНІЄЇ гілки: гроші майбутнього року.
	// Перші дві її не дають — «Авто» валютне (індексу цін країни валюти в
	// застосунку немає), «Ремонт» закритий, — тож без цієї ціль у
	...
	// Пишеться фоновою задачею оновлення НБУ, якої в тесті немає, і не в
	// налаштування, а в app_state — довідник спільний на всі портфелі
	// (0054). Фіксований момент, а не goldenNow: у документ воно потрапляє
	// рядком як є, і будь-яка похідна від часу дата зробила б golden
	// нестабільним.
	if err := st.SetAppState(ctx, "nbu_refreshed_at", "2026-07-15T06:00:00Z"); err != nil {
		t.Fatal(err)
	}
	...
	//  · КАРТКОВА — debt.exit.planned_uah і колонка planned у розкладі
	//    виходу з ліміту. Дата в ПОТОЧНОМУ місяці навмисно: розклад
	//    обривається на місяці, у якому борг сходить у нуль, а тут він
	//    сходить у першому ж, тож витрата в будь-якому пізнішому місяці
	//    лишила б колонку розкладу вічним нулем;
	...
	for _, e := range []string{
		...
		`{"name":"Котел","amount":"3000.00","due_date":"` + string(d(10)) + `","paid_from":"card"}`,
...
var allowedZero = map[string]string{
	// Валюта звітності: порожній ключ означає гривню, і саме цей дефолт
	// фікстура й показує — золотий документ будується в книжковій валюті.
	// Примітка про валюту зʼявляється лише коли просили долар, а курсу ще
	...
	// заради якого правку робили.
	//
	// Робити фікстуру боржником заради одного поля означало б зрушити
	// половину документа — план місяця, стелі подушки, цілей і боргу,
	// частки розкладу й дві задачі наповнення. Саму гілку переповнення
	// тримає TestMandatoryDebtOverflowsIntoMonthPlan.
	"month_plan.debt_from_plan_uah": "непортфельних грошей вистачило на весь обовʼязковий платіж",
	...
	// left_uah виключення БІЛЬШЕ НЕ ПОТРЕБУЄ, і це наслідок фази 0056.
	// Доти борг на цій фікстурі сходив у нуль у першому ж місяці, розклад
	// був одним рядком — тим самим останнім, — і поле лишалось нулем за
	// побудовою. Планова витрата з картки в поточному місяці зменшила
	// гасіння, розклад став довшим, і тепер видно те, заради чого він і
	// існує: борг меншає крок за кроком. Що ОСТАННІЙ крок доводить його до
	// нуля, тримає TestDebtExitScheduleReachesZero.
	...
	// Відклична сходинка подушки на цій фікстурі гаситься на 60-й день,
	// тобто ВСЕРЕДИНІ 90-денного вікна: її тіло вже прийшло потоком у
	// «за 90 днів», і поставити його ще й у «зламне» означало б показати ті
	// самі гроші двічі. Нуль тут — та сама заборона подвійного обліку, що
	// й у locked_uah, а не мовчання фази.
	//
	// Подовжити строк рунги заради одного поля не можна: пара «відклична
	// коротка / безвідклична довша» тримає профіль доступу подушки, де від
	// неї залежать і покриті, і непокриті горизонти. Гілку «резервна рунга
	// потрапила у зламне» тримає TestLiquidityMarksReserveRungs.
	"liquidity.breakable_reserve_uah": "відклична рунга подушки гаситься у вікні — вона вже в «за 90 днів»",
