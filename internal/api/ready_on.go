...
func planParts(src *sources, marks planMarks, today domain.Date,
	...
	for _, f := range src.planFlows {
		...
		// Відмічене цього місяця вже в балансі, і показати його ще раз
		// означало б повести ті самі гроші двічі. Лише поточний місяць:
		// у майбутніх відміток немає за побудовою.
		if _, marked := marks.at(f.ID, today, m); marked && m == 0 {
			continue
		}
		...
		out = append(out, planPart{
			date: date, name: f.Name, uses: f.Uses, weight: w, id: f.ID,
		})
	}
...
// третій читач того самого числа не розходиться з першими двома.
//
...
// # ПОТОЧНИЙ МІСЯЦЬ
//
// Віддає лише НЕДОНЕСЕНЕ (LeftUAH) і лише через ті потоки, чий день ще не
// минув і чия відмітка ще не стоїть. Наслідок треба знати вголос: коли з
// чотирьох виплат лишилась одна, вона несе весь залишок місяця — тобто
// більше за себе саму. Це ціна того, що сума ніг дорівнює LeftUAH; без неї
// стеля подушки розраховувала б на гроші, яких маршрут уже не показує.
//
// # ЧОГО ТУТ НЕМАЄ
//
// Позапланове (ExtraUAH) дати не має — за ним немає потоку, — тож їде
// всередині PlanUAH і трохи піднімає кожну ногу. Напрямок цього розмазування
// лише один: нога потоку, якому подушка заборонена, може везти кілька
// гривень, що подушці насправді дозволені, і вони до неї не дійдуть.
// Перебору не буває — стеля рахується без них.
//
