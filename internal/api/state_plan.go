...
// вікна усереднення, ні індексації. Це відповідь на питання «скільки цей
// потік дає щомісяця, коли він діє» — і саме тому потік, що почнеться за
// пів року, показує тут свою повну ставку, а не половину її через
// дванадцятимісячне вікно. Коли він почнеться, видно в колонці «З».
//
// А от дату КІНЦЯ знає, і асиметрія тут навмисна. «Ще не почався» і «вже
// скінчився» — не дзеркальні випадки: у першого ставка попереду, у другого
// її немає взагалі. Закрита «⇗» стара зарплата, яка й далі показувала б
// 7 600 ₴ щомісяця, підсумовувалась би в «щомісячний дохід» разом із новою
// — тобто застосунок казав би, що ти отримуєш обидві.
//
// Разова дає нуль, і це не пропуск: у разової виплати немає «щомісяця»
...
func planFlowSteadyNative(f store.PlanFlow, today domain.Date, applyShare bool) float64 {
	...
	case "year":
		step = 12
	default: // once і будь-що незнане
		return 0
	}
	amt := float64(f.Amount) / 100 / step
	if applyShare {
		amt *= float64(f.InvestBP) / 10000
	}
	if f.Kind == "expense" {
