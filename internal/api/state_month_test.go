...
func TestReserveMonthShare(t *testing.T) {
	...
	month, fill := reserveMonthShare(set, 0, plan, 0, false, 0, 0)
	...
	month, fill = reserveMonthShare(set, 5000, plan, 5000, false, 0, 0)
	...
	if _, fill = reserveMonthShare(set, 15000, plan, 15000, false, 0, 0); fill != 0 {
	...
	if month, _ = reserveMonthShare(set, 59_000, plan, 0, false, 0, 0); month != 1000 {
		t.Errorf("частка %v, очікували 1000 — більше за розрив не кладуть", month)
	}
	// Ціль зібрано — стеля мовчить незалежно від плану.
	if month, fill = reserveMonthShare(set, 60_000, plan, 0, false, 0, 0); month != 0 || fill != 0 {
	...
	// не входить узагалі).
	if month, fill = reserveMonthShare(set, 0, nil, 0, false, 0, 0); month != 0 || fill != 0 {
