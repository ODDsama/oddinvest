...
type hypothetical struct {
	...
	// rates — КУРСИ, яких сьогодні немає.
	//
	// ЧЕТВЕРТА ПРИРОДА поруч із трьома описаними вище, і питання в неї
	// своє: не «що станеться, якщо це купити» і не «що означають названі
	// цілі», а «що з ЦИМ портфелем зробив би рух курсу, який УЖЕ був».
	// Гіпотезою тут не портфель і не політика, а сам світ.
	//
	// Домішується РАНІШЕ за накладку політики: витрати можуть бути задані
	// у валюті (0038), і перекласти їх треба вже за новим курсом.
	//
	// Знецінення (src.deval) при цьому НЕ рухається, і це рішення, а не
	// недогляд: шок — разовий рух РІВНЯ, а не зміна довгострокового
	// річного темпу. Довід, чому темп із одного епізоду не можна брати за
