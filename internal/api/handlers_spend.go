...
// не чіпає сьогоднішніх грошей.
//
// МЕЖА ТУТ ІНША, НІЖ У ПЛАНІ КУПІВЕЛЬ, і це рішення. Там «зараз» —
// увесь поточний місяць, бо рядок мусив мати куди подітись: у портфелі
// його не було, а сітка прогнозу місячна, тож завтрашня покупка не
// потрапляла нікуди. Тут такої діри немає — майбутня гілка ПОВНА сама
// собою (обидві половини руху лежать у плані), і послаблювати поріг
// нема заради чого: зняття, датоване 28-м, ще не сталося, а гаманець
// показує саме те, що є зараз.
