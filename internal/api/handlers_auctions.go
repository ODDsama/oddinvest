...
func (s *Server) handleAuctionsCurve(w http.ResponseWriter, r *http.Request) {
	...
	for _, p := range cur {
		row := curveRow{
			...
			MinPct: domain.Round2(float64(p.MinBP) / 100),
			MaxPct: domain.Round2(float64(p.MaxBP) / 100),
			Demand: domain.Round2(float64(p.BTCx100) / 100),
			Sold:   domain.Round2(float64(p.SoldMinor) / 100),
		}
		...
		if q, ok := was[p.Currency+"|"+p.Bucket]; ok && q.Date != p.Date {
			row.PrevPct = domain.Round2(float64(q.IncomeBP) / 100)
