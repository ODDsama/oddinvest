...
var Registry = []Def{
	...
	{Key: "reinvest_rank", Str: func(s *state.SettingsDoc) *string { return &s.ReinvestRank },
		Why: "критерій порядку в «Що взяти»"},
	...
	// Ціна користування власною подушкою. У НАБОРИ СТРАТЕГІЙ вона не
	// входить навмисно: набір відповідає на питання «як я вкладаю», а це —
	// про дисципліну, і однакове при будь-якому розподілі. Дописати її
	// десятим питанням означало б питати те, що від стратегії не залежить.
	{Key: "reserve_loan_rate_pct", Num: func(s *state.SettingsDoc) **float64 { return &s.ReserveLoanRatePct },
		Why: "під скільки річних ти позичаєш сам у себе, коли береш із подушки; " +
			"у вже взяту позику ставка потрапила знімком, тож правка діє лише на наступні"},
	...
	{Key: "debt_fill_share_pct", Num: func(s *state.SettingsDoc) **float64 { return &s.DebtFillSharePct },
		Why: "яка частка КАРТКОВИХ грошей місяця (те, що лишається після обовʼязкового " +
			"платежу) варта дострокового погашення; це порада на сторінці боргу, а не " +
			"вирізка — портфельних грошей борг не забирає; порожньо = не пропонувати"},
	// Ключа debt_fill_from тут більше немає: розкладка не веде гроші в борг,
	// тож питання «з яких грошей гасити» стало питанням без механізму.
	// Дострокове погашення тепер робить людина, а стеля на сторінці боргу
	// міряється від КАРТКОВИХ грошей — вибирати їх джерело нема з чого.
	...
	//
	// ЗАСТАРІЛИЙ спільний знак. Відколи знак свій у кожного профілю
	// (app_state, api/handlers_import.go importSinceKey, 2026-09-24), цей
	// ключ лише читається як початкове значення для профілю, у якого свого
	// ще немає, — щоб перше оновлення не перебирало всю історію з нуля.
	{Key: "import_since", Why: "застарілий спільний водяний знак імпорту (початкове значення профілю)"},
}
