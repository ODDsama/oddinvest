...
// KnownSellers — продавці, яких джерело возить: латинський ключ -> назва,
// як її пише сайт.
//
// НАВІЩО СТАЛИЙ ПЕРЕЛІК, КОЛИ СТОРІНКА ВІДДАЄ ЙОГО САМА. Зіставлення
// власного брокера з ключем продавця робиться ДО першого обходу — інакше
// виходить замкнене коло: ціна не потрапляє в квиток без зіставлення, а
// зіставляти нема з чим, поки не набрано цін. Саме в це коло робота й
// потрапила на бойовому: на свіжій базі перелік був порожній.
//
// ЦЕ ЗАСІВ, А НЕ ІСТИНА. Авторитетний перелік лишається на сторінці
// (rawConfig.BrokerNames), і читач цієї мапи мусить зводити її з тим, що
// реально прийшло, — тоді новий продавець у джерела з'явиться сам після
// першого ж обходу, а зниклий просто перестане мати ціни.
var KnownSellers = map[string]string{
	"Privat24":  "Приват 24",
	"Inzhur":    "Inzhur",
	"Univer":    "УНІВЕР",
	"BtcBroker": "БТС Брокер",
	"ICU":       "ICU",
	"Kinto":     "Кінто",
}
