...
var derivedTables = map[string]bool{
	...
	"secrets": true,
	// portfolios — сам вимір, а не його вміст: бекап ЗАВЖДИ одного
	// портфеля і відновлюється в той, на який іде запит (0054). Перелік
	// портфелів у файлі змусив би restore або створювати чужі портфелі,
	// або мовчки їх пропускати.
	"portfolios": true,
	// app_state — те, що належить інсталяції, а не портфелю, і поза
	// бекапом воно з двох різних причин. Робочий стан джоб (час оновлення
	// довідника НБУ, знак аукціонів) — похідне від того самого довідника,
	// що й bonds. Розкладка UI (nav_order) не похідна ні від чого, але
	// бекап переносить ДАНІ портфеля, а не вподобання цієї машини:
	// відновлення копії з ноутбука не має перекладати список на телефоні.
	"app_state": true,
}
