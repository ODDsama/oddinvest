...
func TestImportOverExistingFundPrices(t *testing.T) {
	...
	// Позначки ціни — спільні факти про фонд (0034), а не дані портфеля:
	// відновлення одного портфеля їх не стирає (pruneOrphanFundsIn лишає
	// фонд, у якого є позначки). Доти воно їх стирало — і з ними ціни,
	// вписані руками для сусіднього портфеля.
	if len(got) != 1 || got[0].Fund != "МілТех" {
		t.Errorf("позначка МілТех мала пережити відновлення чужого дампу: %+v", got)
	}
}
