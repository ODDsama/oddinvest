...
type SettingsDoc struct {
	...
	DebtFillFrom     string   `json:"debt_fill_from,omitempty"`
...
type CapitalDelta struct {
	...
	// гаманця, рухи подушки, цілей і пенсійного. Той самий склад, що в
	// «усіх грошах» ціни рішень; переказ між кошиками записується двома
	// ногами й дає нуль сам.
...
type MonthPlan struct {
	...
	// PlanDebtUAH — скільки з PlanUAH дозволено на ДОСТРОКОВЕ погашення
	// боргу (пʼятий кошик plan_flows.uses). Третє число тієї самої
	// природи, і без omitempty з того самого доводу: нуль тут означає
	// заборону, а не відсутність.
	PlanDebtUAH float64 `json:"plan_debt_uah"`
...
type Liquidity struct {
	...
	ReserveUAH float64 `json:"reserve_uah,omitempty"`
	// GoalsUAH — гроші під цілями накопичення. Так само окремим рядком і з
	// того самого доводу: у now_uah їм не місце (інваріант зі звітом про
	// рух коштів), а в locked_uah вони брехали б — ламати нічого не треба,
	// вони просто чекають свого дня.
	//
	...
	// річ, і саме тому в неї є дата, а в подушки немає.
	GoalsUAH  float64 `json:"goals_uah,omitempty"`
