...
type Reserve struct {
	...
	//
	// FillSharePct — стеля, яку задав користувач. FillFromUAH — вільні
	// гроші, від яких її пораховано: готівка на рахунках брокерів. Не весь
	// капітал — продавати папір заради подушки застосунок не радить, бо цін
	// вторинного ринку не знає. FillNowUAH — скільки варто відкласти зараз,
	// тобто МЕНШЕ з розриву й стелі.
	//
	// Три числа, а не одне, з тієї ж причини, з якої достатність міряється
	// двома мірами вище: «відкласти 12 000 ₴» без «звідки» — це число, яке
	// нема чим перевірити, а таких застосунок не показує.
	//
	// Усі три з'являються разом і лише коли FillNowUAH > 0. Вимкнена стеля,
	// зібрана ціль і порожні рахунки дають однаковий результат — сказати
	// нема чого, — а три нулі в документі читались би як «механізм працює і
	// радить нуль».
...
type MonthPlan struct {
	...
	// ReserveUAH — скільки з місячних грошей іде в подушку: та сама стеля
	// ReserveFillSharePct, що керує Reserve.FillNowUAH, але прикладена до
	// НОВИХ грошей місяця, а не до готівки на рахунках. Обрізана розривом
	// резерву: більше, ніж бракує до цілі, туди не кладуть.
	ReserveUAH float64 `json:"reserve_uah,omitempty"`
