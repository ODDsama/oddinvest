...
type Doc struct {
	...
	//
	// Входить у капітал і валютні частки, але НЕ в купівельну спроможність і
	// НЕ в зведені дохідності: з НПФ не приходить нічого до 50 років, тож
	// пропонувати його як напрямок для прибулого купона було б нечесно тим
	// самим, чого застосунок уникає всюди.
