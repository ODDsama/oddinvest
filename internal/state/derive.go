...
func deriveGoals(doc *Doc, in DeriveInput) {
	...
	for _, g := range in.Goals {
		...
		if row.DoneDate != "" {
			...
			// Вирок — ОСТАННІМ, і це не стиль, а порядок залежності: він
			// стоїть на майбутній ціні, а її рахує крок вище. Доти
...
// GoalsGapUAH — скільки цілям накопичення разом ще бракує, грн-екв.
//
// Те саме правило, що в deriveGoals: розрив лише додатний, закрита ціль не
// рахується (гроші пішли на річ, заради якої їх збирали). Живе тут, поруч
// із деривацією, саме щоб правило лишилось одним — читач у нього другий і
// далекий: прогноз (state_projection.go), який мусить знати, доки взагалі
// триматимуться стелі наповнення, і робить це ДО того, як Derive побудує
// картки цілей.
func GoalsGapUAH(goals []GoalInput) float64 {
	var sum float64
	for _, g := range goals {
		if g.DoneDate != "" {
			continue
...
func GoalsFill(set *SettingsDoc, goals []Goal, planUAH float64, debtPressure bool) {
	...
	for i := range goals {
		...
		// встигнути: два числа про одну ціль, які суперечать одне одному.
		need := g.RequiredPaceUAH()
		if g.DueDate == "" || need <= 0 {
			need = g.GapPaceUAH()
		}
		// Стеля потреби — розрив у ТІЙ САМІЙ лінійці, що й темп. Обрізати
		// майбутній темп сьогоднішнім розривом означало б зрізати саме ту
		// добавку, заради якої вирок і переїхав на майбутню ціну.
...
func deriveGoalPace(row *Goal, today domain.Date) {
	...
	// її на цьому кроці ще немає. Правило лишилось те саме — нульовий темп
	// теж відставання: ціль із датою, у яку нічого не кладуть, не
	// збереться ніколи.
}

// RequiredPaceUAH — темп, ПРОТИ ЯКОГО ВИНОСИТЬСЯ ВИРОК «встигаю».
//
// Доти вироком був RequiredUAH — розрив у сьогоднішніх грошах, поділений
// на місяці, — і це порівнювало майбутній дедлайн із сьогоднішньою ціною.
// Обидві сторони стояли в одних грошах, тобто внутрішньо все сходилось; не
// сходились ДАТИ. Ціль купують у рік дедлайну, і на десятирічній цілі при
// 10.7%/рік те саме коштує втричі — а екран при цьому казав «темп
// тримається».
//
// Тепер вирок стоїть на майбутній ціні, коли її є з чого порахувати
// (гривнева ціль, є ряд ІСЦ, є дедлайн попереду). Валютна ціль лишається
// на старому темпі, і це не недогляд: індексу цін країни валюти в
// застосунку немає, а вигадати його означало б поставити найважливіше
// число на здогад.
//
// СЬОГОДНІШНІЙ ТЕМП НЕ ЗНИКАЄ. RequiredUAH лишається в документі й у
...
//
// Потрібен рівно там, де темп чимось обмежують: стеля наповнення не має
// права просити більше, ніж ціль узагалі коштує, — але «коштує» мусить
// бути в тій самій лінійці, що й темп. Обмежити майбутній темп
...
func deriveGoalFuture(row *Goal, inflationPct float64) {
	...
	//
	// Доти стояло «зібране не індексується», і майбутній розрив рахувався
	// як майбутня ціна мінус сьогоднішні гроші. Для цілі в шухляді це було
	// правильно випадково: у неї ставка нульова. Відколи ціль може лежати
	// на вкладі (0062), те саме припущення почало вимагати з неї внесків,
	// ніби вклад нічого не дає.
	//
	// RatePct — ВИМІРЯНА ставка цих грошей, чистими після податку, і нуль у
	// ній теж вимір: журнал цілі відсотків не нараховує. Тому при нулі
	...
	if gap := row.TargetFutureNative - grown; gap > 0 {
		...
		// явно: коли (і якщо) охорона зміниться, це місце мусить упасти в
		// очі, а не мовчки почати міряти долари гривнями.
		row.GapFutureUAH = row.GapFutureNative
		row.RequiredFutureUAH = row.RequiredFutureNative
	}
}

// annuityFactor — у скільки разів виросте потік «одиниця щомісяця» за
// months місяців під місячну ставку r. При r == 0 це просто кількість
// місяців, і саме тому формула не розгалужується десь вище: нульова
// ставка тут не окремий випадок, а законне значення.
func annuityFactor(r float64, months int) float64 {
	if months <= 0 {
		return 0
	}
	if math.Abs(r) < 1e-12 {
	...
	return (math.Pow(1+r, float64(months)) - 1) / r
}
