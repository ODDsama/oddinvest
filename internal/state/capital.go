...
// лише читається.
type Capital struct {
	// BondsUAH — ОВДП у грн-екв., усі валюти разом: НОМІНАЛ + НАКОПИЧЕНИЙ
	// КУПОН (рішення власника 2026-09-22 — так само, як їх оцінює XIRR).
	// Сам номінал — у nominal_uah_eq документа.
	BondsUAH Money
	// BondsAccruedUAH — накопичений купон, що сидить у BondsUAH. Окремо
	// лише для одного читача: старт проєкції його ВІДНІМАЄ, бо симуляція
	// платить майбутні купони повністю й інакше порахувала б цей двічі.
	BondsAccruedUAH Money
	// DepositsAccruedUAH — накопичені (ще не виплачені) нетто-відсотки
