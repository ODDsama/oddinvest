...
// Major — курс зі сховища як звичайне число, БЕЗ прив'язки до валюти.
//
// RateMajor поруч відповідає на питання «який курс у цієї валюти
// СЬОГОДНІ» і тому шукає в мапі чинних курсів. Точка ІСТОРІЇ такої мапи
// не має: вона вже прийшла зі сховища числом, і питання про неї одне —
// перевести з внутрішнього ×10⁴ у те, що бачить людина.
//
// Без цієї функції історія курсів ділилась би на масштаб у місці
// виклику — тобто рівно тим способом, через який ×10⁴ уже одного разу
// розповзся по шести файлах (див. RateMajor вище й fx-boundary у
// Makefile).
func Major(e4 int64) float64 { return float64(e4) / RateScale }
