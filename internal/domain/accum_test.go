...
// TestAccumMoneyIsReachableAfterClose — гроші закритого фонду доступні
// декумуляції, а до закриття ні.
//
// Замкнене достроково не продається, і накопичувальний фонд успадкував
// це правило. Але після закриття він уже не фонд, а звичайна готівка, і
// не пустити до неї означало б «портфель вичерпався», маючи гроші на
// рахунку.
func TestAccumMoneyIsReachableAfterClose(t *testing.T) {
	s := Sleeve{
		Currency: "UAH", Rate0: 1,
		Accum: []Accum{{Value0: 12000, Cost0: 12000, CloseM: 6}},
	}
	// Знімаємо 1000 на місяць: до шостого місяця брати нема з чого.
	if m := DrawdownMonths([]Sleeve{s}, 0, 1000, 60); m != 1 {
		t.Errorf("до закриття зняття мало впертись одразу, маємо місяць %d", m)
	}
	// Той самий фонд, який закрився ВЧОРА (гроші вже в рукаві), тримає
	// зняття всі дванадцять місяців.
	open := Sleeve{Currency: "UAH", Rate0: 1, Cash0: 12000}
	if m := DrawdownMonths([]Sleeve{open}, 0, 1000, 60); m != 13 {
		t.Errorf("12000 під 1000/міс мали вичерпатись на 13-му, маємо %d", m)
	}
}
