...
type FundOp struct {
	...
	PairID int64
	// KeepPrice — операція НЕ приносить ціни: якщо в позиції ціна вже є,
	// LastPrice і LastPriceDate лишаються тими, що були.
	//
	// ЩО ЦЕ ЛАГОДИТЬ. stepPosition ставив ціну на КОЖНІЙ купівлі, а
	// MarketValue множить цю єдину ціну на ВЕСЬ залишок. Тож гіпотетична
	// покупка (план купівель, POST /api/whatif) переоцінювала весь наявний
	// пакет за ціною з рядка плану:
	//
	//	ΔФонди = q × P_плану + Qty₀ × (P_плану − P_позиції)
	//
	// Другий доданок не має стосунку до того, що ти купуєш, і не обмежений
	// сумою покупки — він росте з розміром пакета, який у тебе вже є. На
	// живих даних покупка на 46,24 ₴ додавала 352,60 ₴ капіталу.
	//
	// Розбіжність цін виникала двома каналами: задана вручну «Ціна за
	// штуку» і — навіть без неї — округлення до копійки в fundUnitCost,
	// тоді як LastPrice тримає чотири знаки (див. її опис вище: «на 1738
	// сертифікатах розходиться на гривні»).
	//
	// ЩО МАЄ БУТИ НАТОМІСТЬ: купівля вище ринку коштує рівно переплату,
	// q × (P_плану − P_позиції), і нічого більше — так само, як НКД у
	// облігації. Гіпотеза «що як я куплю» не є ринковою ціною.
	//
	// ЧОМУ ПОЛЕ НЕ ДОЇДЕ ДО СХОВИЩА, і чому тут немає json:"-". FundOp не
	// має ЖОДНОГО структурного тега й ніде не серіалізується: /api/funds
	// збирає власний рядок, а бекап має власний тип BackupFundOp.
	// AddFundOp і UpdateFundOp перелічують колонки літерально, ListFundOps
	// сканує фіксований SELECT — тобто з бази це поле завжди приходить
	// нулем. Тег натякав би, що структура кудись серіалізується, а це
	// неправда. Межу тримає grep у make whatif-boundary.
	KeepPrice bool
}
...
//
// Сторож KeepPrice стоїть ЛИШЕ на купівлі. У продажу та сама безумовна
// переоцінка, і це не недогляд: гіпотетичного продажу застосунок не
// будує ніде — у store.PlanBuy немає такого виду, а конвертаційна пара
...
func stepPosition(p *FundPosition, op FundOp) {
	...
	case FundBuy:
		...
		//
		// Питаємо саме про ЗАЛИШОК, бо правило дослівно таке: гіпотеза не
		// має права переоцінювати те, чим ти ВОЛОДІЄШ. Немає залишку —
		// немає чого переоцінювати, і ціна покупки стає єдиним, що про
		// цей фонд узагалі відомо.
		//
		// Ані LastPrice, ані LastPriceDate тут не годяться, і це не
		// теорія: позиція, з якої все продано, лишається з Qty == 0 і
		// СТАРОЮ ціною (продаж її теж ставить). За ціною сторож вирішив
		// би, що переоцінювати є що, і нові сертифікати дістали б ціну
		// давно проданих — на тесті це коштувало 990 ₴ з 1000.
		if op.Qty > 0 && (!op.KeepPrice || held <= 0) {
