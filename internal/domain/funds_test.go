...
// TestNetOfTaxTakesTaxWhenItIsActuallyTaken — податок береться тоді, коли
// його справді беруть, і це дає різні числа.
//
// Безстроковий фонд платить виплатами, і кожна оподатковується одразу:
// ставка просто меншає на податок. Строковий накопичує дохід усередині й
// платить один раз у кінці — тоді податок б'є по СУМАРНОМУ приросту.
// Оподаткувати щороку було б неправильно: держава не бере податок із
// паперу, якого ти не продавав.
func TestNetOfTaxTakesTaxWhenItIsActuallyTaken(t *testing.T) {
	// Безстроковий: 20% під 14% податку — просто 17.2%.
	if got := NetOfTax(20, 14, 0); math.Abs(got-17.2) > 0.01 {
		t.Errorf("безстроковий: %v, очікували 17.2", got)
	}
	// Строковий на три роки: 20.51% це ×1.75, приріст 0.75, після 14%
	// лишається 0.645 → ×1.645 → 18.05% річних.
	got := NetOfTax(20.51, 14, 3)
	if math.Abs(got-18.05) > 0.02 {
		t.Errorf("строковий на 3 роки: %v, очікували ≈18.05", got)
	}
	// І він мусить бути ВИЩИМ за щорічне оподаткування тієї самої
	// ставки — саме тому строк і враховується.
	if yearly := NetOfTax(20.51, 14, 0); got <= yearly {
		t.Errorf("податок у кінці строку (%v) не може бути гіршим за щорічний (%v)", got, yearly)
	}
	// Без податку ставка не чіпається взагалі.
	if got := NetOfTax(20.51, 0, 3); got != 20.51 {
