...
// XIRRMinMoneyDays, XIRRMin, XIRRMax — коли річну ставку ВЗАГАЛІ можна
// показувати: з якого середньозваженого віку грошей (днів) і в якій смузі
// (частка: −95 % … +100 %). Три дні тримання з приростом 0,4 % дають 60 %
// річних — це не дохідність, а арифметика ділення на малий строк; а XIRR на
// коротких і рваних потоках буває збіжним і при цьому нікчемним.
//
// ОДНЕ місце на всі чотири читачі: зведений XIRR і реалізований результат
// (engine/state_builder.go, state_xirr.go), журнал рішень (handlers_decisions.go)
// і дохідність фонду (funds.go). Доти кожен тримав свій літерал −0.95/1.0,
// і перший же, хто посунув би смугу в одному місці, розвів би цифри на
// сусідніх картках.
const (
	XIRRMinMoneyDays = 30
	XIRRMin          = -0.95
...
// XIRRPlausible — чи ставка (частка, 0.1 = 10 %) лежить у смузі, якій
// можна вірити.
func XIRRPlausible(r float64) bool { return r >= XIRRMin && r <= XIRRMax }
...
//
// Робить це engine/state_xirr.go, і саме там, а не тут: курс на дату — це
