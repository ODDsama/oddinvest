...
func TestParsePlanUsesCanonical(t *testing.T) {
	cases := []struct {
		...
		// А неповний перелік звужує — і саме в цьому пастка, про яку
		// застерігає шапка: «повний» перелік застаріває щоразу, коли
		// множина кошиків МІНЯЄТЬСЯ. Так уже було двічі: 0046 повернула до
		// порожнього рядки, записані «скрізь» до появи боргу, а 0058
		// вичистила сам борг, коли він перестав бути кошиком.
		{"неповний перелік звужує",
			[]string{"reserve", "goals"}, "reserve,goals"},
	}
	for _, c := range cases {
