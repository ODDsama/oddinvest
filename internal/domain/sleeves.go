...
type Sleeve struct {
	...
	//
	// Це ЄДИНИЙ флет-внесок: ним рахуються гіпотетичні сценарії
	// («що якщо вносити X/міс»), і саме його підбирає бісекція в
	// RequiredMonthlySleeves.
	ContribUAH float64
	// ContribByMonth — РЕАЛЬНИЙ план: скільки гривень цей рукав отримує
	// на місяці m (індекс m-1), поверх ContribUAH. nil або коротший за m
	// = нуль на цьому місяці — план не обіцяє нічого далі обрію, який
	// сам собі задав.
	//
	// Окреме поле, а не заміна ContribUAH, навмисно: ContribUAH лишається
	// «а що якщо» (сценарії, бісекція шукає, скільки ще потрібно), а
	// ContribByMonth — «що вже є» (справжні джерела доходу з датами).
	// Разом вони й дають повну відповідь: RequiredMonthlySleeves, шукаючи
	// флет-надбавку до сценарію на сленевах, що вже несуть ContribByMonth
	// із плану, автоматично рахує «скільки бракує ПОНАД план» — жодної
	...
	Accum []Accum
	Dist  []Dist
	// Lock — планова дія «замкнути суму на строк»: місяць (індекс 1..) →
	// сума в нативній валюті, яка ЦЬОГО місяця переходить із ліквідного
	// (invested, потім cash) у locked. Купон і повернення тіла замка —
	// це ЗВИЧАЙНІ записи в Coupon/Redeem, нового поля під них не треба:
	// замок економічно не відрізняється від вкладу чи облігації, щойно
	// гроші в ньому опинились.
	//
	// Це лишається правдою рівно для того, що в Lock ЛИШИЛОСЬ: ОВДП, тіло
	// вкладу, розподільний фонд. Накопичувальний звідси пішов у Spend — з
	// того самого доводу, який записано десятьма рядками вище про Accum:
	// у замкненому він лежав би цеглиною, а весь його дохід саме в
	// зростанні.
	Lock map[int]float64
	// Spend — місяць (індекс 1..) → сума в нативній валюті, яка ЦЬОГО
	// місяця йде з ліквідного боку й НЕ стає замкненою.
	//
	// Друга половина руху, першою в якому є Accum.ContribByMonth. Обидві
	// заводяться одним викликом і ніколи окремо: сама по собі кожна або
	// губить гроші мовчки, або створює їх із нічого, і в обох випадках
	// числа лишаються правдоподібними.
	//
	// ОКРЕМЕ ПОЛЕ, а не прапорець на Lock, бо це різні економіки. Замок
	// кладе гроші туди, де вони лежать за номіналом і платять купон;
	// Spend віддає їх позиції, яка росте й не платить нічого. Доти
	// планована купівля накопичувального фонду була змушена прикидатись
	// замком — і РОБИЛА ПРОГНОЗ ГІРШИМ, бо тіло замка не росте, а купон
	// простий (див. state_plan_buys.go).
...
// baseContribUAH — сумарний гривневий внесок рукава на місяці m: флет
// ContribUAH (сценарій/бісекція) плюс ContribByMonth[m-1] (справжній
// план), якщо є.
func (s Sleeve) baseContribUAH(m int) float64 {
	total := s.ContribUAH
	if s.ContribByMonth != nil && m >= 1 && m-1 < len(s.ContribByMonth) {
		total += s.ContribByMonth[m-1]
	}
	return total
}
...
// RequiredMonthlySleeves — бісекцією підбирає ФЛЕТ-НАДБАВКУ (ContribUAH)
// понад те, що вже закладено в рукавах, за якої капітал у сьогоднішніх
// гривнях сягає цілі рівно на місяці months. Внесок розкладається по
// рукавах у тих самих пропорціях, що й зараз, — щоб відповідь була про
// суму, а не про зміну стратегії.
//
// «Понад те, що вже закладено» — не застереження, а вбудована поведінка:
// бісекція чіпає лише ContribUAH кожного рукава (withContrib нижче),
// ContribByMonth лишається незайманим. Тож коли sleeves несуть справжній
// план (ContribByMonth заповнений через sleeveFactory), результат
// автоматично стає «скільки бракує ПОНАД план» — жодної окремої функції
// для цього не знадобилось. На порожньому плані (ContribByMonth усюди
// нуль) це рівно стара поведінка: «скільки треба з нуля».
