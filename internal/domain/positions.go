...
func Positions(bonds map[string]Bond, payments []Payment, lots []Lot, sales []Sale, asOf Date) ([]Position, error) {
	...
	for isin, a := range byISIN {
		...
		// Погашений папір — уже не позиція. Номінал повернувся на рахунок
		// (погашення проходить звичайною виплатою), а сам лот нікуди не
		// дівається: продажу не було, тож RemainingQtyNow і далі дає повну
		// кількість. Доки цього фільтра не було, ті самі гроші стояли і в
		// nominal_uah_eq, і в account_uah — тобто в капіталі ДВІЧІ.
		//
		// Драбина (Ladder) і YTM-мапа таких паперів не бачать уже давно;
		// тут та сама умова, і тепер вона одна на всіх.
		//
		// Порожню дату перевіряємо ОКРЕМО й навмисно: Date.Before — це
		// порівняння рядків, тож "" «раніше» за будь-яку дату. Папери, яких
