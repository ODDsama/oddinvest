...
// період еквівалентний купону). Дзеркально на боці купівлі: виплата,
// датована на/до дати купівлі, належала попередньому власнику —
// накопичений купон уже сплачено йому в брудній ціні, тож ми її не
// отримуємо (див. ту саму умову в sale.go). Без цього купон до дати
// купівлі помилково зараховувався як дохід і роздував XIRR.
func HolderQty(lot Lot, sales []Sale, on Date) int64 {
	if !on.After(lot.BuyDate) {
		return 0
	}
	q := lot.Qty
	for _, s := range sales {
