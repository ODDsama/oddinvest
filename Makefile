...
# Те саме ганяє джоба `ui` у ci.yml. Порядок від дешевого до дорогого: чи
# резолвиться кожен імпорт (модуль із синтаксичною помилкою не імпортується,
# тож окремий `node --check` зайвий), чи визначене кожне вжите імʼя, шар
# токенів CSS, адреси.
#
# eslint ставиться через npx і в застосунок не потрапляє — правило «жодних
# залежностей у web/» не порушене. package.json у web/ більше немає: Node
# 22+ сам розпізнає ES-модулі, а вшивався той файл у бінарник разом зі
# статикою.
ui:
